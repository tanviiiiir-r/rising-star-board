...
import { DailyListingRow } from "@/components/board/DailyListingRow";
import { UtcCountdown } from "@/components/board/UtcCountdown";
import { buttonVariants } from "@/components/ui/button";
import type { DailyBoardPreview } from "@/lib/board.functions";
      ...
      {board.listings.length === 0 ? (
        ...
        <ul className="mt-4 flex flex-col gap-2">
          {board.listings.map((listing) => (
            <li key={listing.id}>
              <DailyListingRow listing={listing} date={board.date} />
            </li>
          ))}
        </ul>
      )}
      ...
      <div className={cn("mt-4 flex flex-col gap-2 sm:flex-row", !board.live && "sm:justify-end")}>
        {board.live ? (
          <Link
            ...
            search={{ board: "today" }}
            hash="claim"
            className={cn(buttonVariants({ size: "lg" }), "h-10 flex-1 rounded-full")}
          >
            Claim a rank
          </Link>
        ) : null}
