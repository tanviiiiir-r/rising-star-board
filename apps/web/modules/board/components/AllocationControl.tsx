...
import { RANKING, assertAllocationAmount } from "@repo/database/ranking";
...
export function AllocationControl({
	...
	const mutation = useMutation({
		...orpc.listings.setAllocation.mutationOptions(),
		onSuccess: async () => {
		...
		<div className="mt-3 rounded-lg border border-border bg-surface/60 p-3">
			...
			<div className="mt-2 flex flex-wrap items-center gap-2">
				...
				<Button
					...
					variant="primary"
					disabled={!canSave}
					onClick={() => mutation.mutate({ listingId, cents: draftCents })}
				>
				...
				{allocationCents > 0 ? (
					<Button
						...
						onClick={() => mutation.mutate({ listingId, cents: 0 })}
					>
						Release all
					</Button>
				) : null}
